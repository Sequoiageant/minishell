...
/*   Updated: 2020/07/16 17:27:23 by grim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_exec_cmd(t_list *cmd_elem, char **env_tab)
{
	...
	if (cmd->cmd_path)
	{
		...
		{
			ft_putstr_fd(cmd->argv[0], 2);
			ft_putendl_fd(": Is a directory or Permission is denied", 2);
			exit(126);
		}
	}	
	else // une recherche a été faite dans PATH, pas de commande correspondante
	{
		ft_putstr_fd(cmd->argv[0], 2);
